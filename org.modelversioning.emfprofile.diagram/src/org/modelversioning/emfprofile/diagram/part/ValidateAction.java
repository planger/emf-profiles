...
import org.modelversioning.emfprofile.diagram.providers.EMFProfileValidationDecoratorProvider;
import org.modelversioning.emfprofile.diagram.providers.EMFProfileValidationProvider;
...
public class ValidateAction extends Action {
	...
	public static void runValidation(View view) {
		try {
			if (EMFProfileDiagramEditorUtil.openDiagram(view.eResource())) {
	...
	public static void runValidation(DiagramEditPart diagramEditPart, View view) {
		...
		EMFProfileValidationProvider.runWithConstraints(txDomain,
				new Runnable() {
	...
	private static void validate(DiagramEditPart diagramEditPart, View view) {
		...
		EMFProfileValidationDecoratorProvider.refreshDecorators(view);
	...
	private static void createMarkers(View target, IStatus validationStatus,
		...
		EMFProfileDiagramEditorUtil.LazyElement2ViewMap element2ViewMap = new EMFProfileDiagramEditorUtil.LazyElement2ViewMap(
				diagramEditPart.getDiagramView(), collectTargetElements(
						rootStatus, new HashSet<EObject>(), allStatuses));
		for (Iterator it = allStatuses.iterator(); it.hasNext();) {
			IConstraintStatus nextStatus = (IConstraintStatus) it.next();
			View view = EMFProfileDiagramEditorUtil.findView(diagramEditPart,
	...
	private static void createMarkers(View target,
		...
		for (Iterator it = emfValidationStatus.getChildren().iterator(); it
			...
			if (data != null && !data.isEmpty()
				...
				View view = EMFProfileDiagramEditorUtil.findView(
